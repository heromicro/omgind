...
func process(dc *DeepCopier, dst interface{}, src interface{}, args ...Options) error {
	...
	for _, f := range srcFieldNames {
		...
		if slice.ContainsString(dc.excluded, f) {
			//fmt.Printf(" -- == === excluded field: %s \n", f)
			continue
		}
		var (
		...
		if _, ok := tagOptions[SkipOptionName]; ok {
			if !slice.ContainsString(dc.included, f) {
				continue
			}
		}
	...
	for _, m := range srcMethodNames {
		if slice.ContainsString(dc.excluded, m) {
			continue
		}
		...
		if _, ok := opts[SkipOptionName]; ok {
			if !slice.ContainsString(dc.included, m) {
				continue
			}
		}
