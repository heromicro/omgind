...
func NewOrgDeptSrv(entCli *mainent.Client, deptRepo *repo.OrgDept, q queue.Queuer, c *worker.Consumer) *OrgDept {
...
func (s *OrgDept) ProcessTask(ctx context.Context, t *asynq.Task) error {
	...
	parent, err := s.EntCli.OrgDept.Query().Where(orgdept.IDEQ(id)).WithChildren(func(sdq *mainent.OrgDeptQuery) {
