...
func (xdc *XxxDemoCreate) check() error {
	if _, ok := xdc.mutation.IsDel(); !ok {
		return &ValidationError{Name: "is_del", err: errors.New(`entscheme: missing required field "XxxDemo.is_del"`)}
	}
	if v, ok := xdc.mutation.Memo(); ok {
		if err := xxxdemo.MemoValidator(v); err != nil {
			return &ValidationError{Name: "memo", err: fmt.Errorf(`entscheme: validator failed for field "XxxDemo.memo": %w`, err)}
		}
	}
	if _, ok := xdc.mutation.Sort(); !ok {
		return &ValidationError{Name: "sort", err: errors.New(`entscheme: missing required field "XxxDemo.sort"`)}
	}
	if _, ok := xdc.mutation.IsActive(); !ok {
		return &ValidationError{Name: "is_active", err: errors.New(`entscheme: missing required field "XxxDemo.is_active"`)}
	}
	if _, ok := xdc.mutation.Code(); !ok {
		return &ValidationError{Name: "code", err: errors.New(`entscheme: missing required field "XxxDemo.code"`)}
	}
	if v, ok := xdc.mutation.Code(); ok {
		if err := xxxdemo.CodeValidator(v); err != nil {
			return &ValidationError{Name: "code", err: fmt.Errorf(`entscheme: validator failed for field "XxxDemo.code": %w`, err)}
		}
	}
	if _, ok := xdc.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`entscheme: missing required field "XxxDemo.name"`)}
	}
	if v, ok := xdc.mutation.Name(); ok {
		if err := xxxdemo.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`entscheme: validator failed for field "XxxDemo.name": %w`, err)}
		}
	}
	if v, ok := xdc.mutation.ID(); ok {
		if err := xxxdemo.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`entscheme: validator failed for field "XxxDemo.id": %w`, err)}
		}
	}
	return nil
}
...
func (u *XxxDemoUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for XxxDemoCreate.OnConflict")
	}
...
func (u *XxxDemoUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("entscheme: XxxDemoUpsertOne.ID is not supported by MySQL driver. Use XxxDemoUpsertOne.Exec instead")
	}
...
func (u *XxxDemoUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("entscheme: OnConflict was set for builder %d. Set it on the XxxDemoCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for XxxDemoCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
