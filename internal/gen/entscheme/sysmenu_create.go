...
func (smc *SysMenuCreate) check() error {
	if _, ok := smc.mutation.IsDel(); !ok {
		return &ValidationError{Name: "is_del", err: errors.New(`entscheme: missing required field "SysMenu.is_del"`)}
	}
	...
	if _, ok := smc.mutation.Sort(); !ok {
		return &ValidationError{Name: "sort", err: errors.New(`entscheme: missing required field "SysMenu.sort"`)}
	}
	if _, ok := smc.mutation.IsActive(); !ok {
		return &ValidationError{Name: "is_active", err: errors.New(`entscheme: missing required field "SysMenu.is_active"`)}
	}
	if _, ok := smc.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`entscheme: missing required field "SysMenu.name"`)}
	}
	...
	if _, ok := smc.mutation.Icon(); !ok {
		return &ValidationError{Name: "icon", err: errors.New(`entscheme: missing required field "SysMenu.icon"`)}
	}
	...
	if _, ok := smc.mutation.Router(); !ok {
		return &ValidationError{Name: "router", err: errors.New(`entscheme: missing required field "SysMenu.router"`)}
	}
	...
	if _, ok := smc.mutation.IsShow(); !ok {
		return &ValidationError{Name: "is_show", err: errors.New(`entscheme: missing required field "SysMenu.is_show"`)}
	}
	...
	if _, ok := smc.mutation.Level(); !ok {
		return &ValidationError{Name: "level", err: errors.New(`entscheme: missing required field "SysMenu.level"`)}
	}
	...
	if v, ok := smc.mutation.ID(); ok {
		if err := sysmenu.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`entscheme: validator failed for field "SysMenu.id": %w`, err)}
		}
	}
	return nil
}
...
func (u *SysMenuUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysMenuCreate.OnConflict")
	}
...
func (u *SysMenuUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("entscheme: SysMenuUpsertOne.ID is not supported by MySQL driver. Use SysMenuUpsertOne.Exec instead")
	}
...
func (u *SysMenuUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("entscheme: OnConflict was set for builder %d. Set it on the SysMenuCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysMenuCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
