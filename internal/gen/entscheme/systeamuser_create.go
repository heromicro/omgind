...
func (stuc *SysTeamUserCreate) check() error {
	if _, ok := stuc.mutation.Sort(); !ok {
		return &ValidationError{Name: "sort", err: errors.New(`entscheme: missing required field "SysTeamUser.sort"`)}
	}
	if _, ok := stuc.mutation.IsActive(); !ok {
		return &ValidationError{Name: "is_active", err: errors.New(`entscheme: missing required field "SysTeamUser.is_active"`)}
	}
	if v, ok := stuc.mutation.Memo(); ok {
		if err := systeamuser.MemoValidator(v); err != nil {
			return &ValidationError{Name: "memo", err: fmt.Errorf(`entscheme: validator failed for field "SysTeamUser.memo": %w`, err)}
		}
	}
	if _, ok := stuc.mutation.IsDel(); !ok {
		return &ValidationError{Name: "is_del", err: errors.New(`entscheme: missing required field "SysTeamUser.is_del"`)}
	}
	if _, ok := stuc.mutation.TeamID(); !ok {
		return &ValidationError{Name: "team_id", err: errors.New(`entscheme: missing required field "SysTeamUser.team_id"`)}
	}
	if v, ok := stuc.mutation.TeamID(); ok {
		if err := systeamuser.TeamIDValidator(v); err != nil {
			return &ValidationError{Name: "team_id", err: fmt.Errorf(`entscheme: validator failed for field "SysTeamUser.team_id": %w`, err)}
		}
	}
	if _, ok := stuc.mutation.UserID(); !ok {
		return &ValidationError{Name: "user_id", err: errors.New(`entscheme: missing required field "SysTeamUser.user_id"`)}
	}
	if v, ok := stuc.mutation.UserID(); ok {
		if err := systeamuser.UserIDValidator(v); err != nil {
			return &ValidationError{Name: "user_id", err: fmt.Errorf(`entscheme: validator failed for field "SysTeamUser.user_id": %w`, err)}
		}
	}
	if v, ok := stuc.mutation.ID(); ok {
		if err := systeamuser.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`entscheme: validator failed for field "SysTeamUser.id": %w`, err)}
		}
	}
	if _, ok := stuc.mutation.UserID(); !ok {
		return &ValidationError{Name: "user", err: errors.New(`entscheme: missing required edge "SysTeamUser.user"`)}
	}
	if _, ok := stuc.mutation.TeamID(); !ok {
		return &ValidationError{Name: "team", err: errors.New(`entscheme: missing required edge "SysTeamUser.team"`)}
	}
	return nil
}
...
func (u *SysTeamUserUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysTeamUserCreate.OnConflict")
	}
...
func (u *SysTeamUserUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("entscheme: SysTeamUserUpsertOne.ID is not supported by MySQL driver. Use SysTeamUserUpsertOne.Exec instead")
	}
...
func (u *SysTeamUserUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("entscheme: OnConflict was set for builder %d. Set it on the SysTeamUserCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysTeamUserCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
