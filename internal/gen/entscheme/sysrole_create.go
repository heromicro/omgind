...
func (src *SysRoleCreate) check() error {
	if _, ok := src.mutation.IsDel(); !ok {
		return &ValidationError{Name: "is_del", err: errors.New(`entscheme: missing required field "SysRole.is_del"`)}
	}
	if _, ok := src.mutation.IsActive(); !ok {
		return &ValidationError{Name: "is_active", err: errors.New(`entscheme: missing required field "SysRole.is_active"`)}
	}
	if _, ok := src.mutation.Sort(); !ok {
		return &ValidationError{Name: "sort", err: errors.New(`entscheme: missing required field "SysRole.sort"`)}
	}
	if v, ok := src.mutation.Memo(); ok {
		if err := sysrole.MemoValidator(v); err != nil {
			return &ValidationError{Name: "memo", err: fmt.Errorf(`entscheme: validator failed for field "SysRole.memo": %w`, err)}
		}
	}
	if _, ok := src.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`entscheme: missing required field "SysRole.name"`)}
	}
	if v, ok := src.mutation.Name(); ok {
		if err := sysrole.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`entscheme: validator failed for field "SysRole.name": %w`, err)}
		}
	}
	if v, ok := src.mutation.ID(); ok {
		if err := sysrole.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`entscheme: validator failed for field "SysRole.id": %w`, err)}
		}
	}
	return nil
}
...
func (u *SysRoleUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysRoleCreate.OnConflict")
	}
...
func (u *SysRoleUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("entscheme: SysRoleUpsertOne.ID is not supported by MySQL driver. Use SysRoleUpsertOne.Exec instead")
	}
...
func (u *SysRoleUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("entscheme: OnConflict was set for builder %d. Set it on the SysRoleCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysRoleCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
