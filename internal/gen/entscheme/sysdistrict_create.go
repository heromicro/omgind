...
func (sdc *SysDistrictCreate) check() error {
	if _, ok := sdc.mutation.IsDel(); !ok {
		return &ValidationError{Name: "is_del", err: errors.New(`entscheme: missing required field "SysDistrict.is_del"`)}
	}
	if _, ok := sdc.mutation.Sort(); !ok {
		return &ValidationError{Name: "sort", err: errors.New(`entscheme: missing required field "SysDistrict.sort"`)}
	}
	if _, ok := sdc.mutation.IsActive(); !ok {
		return &ValidationError{Name: "is_active", err: errors.New(`entscheme: missing required field "SysDistrict.is_active"`)}
	}
	if v, ok := sdc.mutation.Name(); ok {
		if err := sysdistrict.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`entscheme: validator failed for field "SysDistrict.name": %w`, err)}
		}
	}
	if v, ok := sdc.mutation.NameEn(); ok {
		if err := sysdistrict.NameEnValidator(v); err != nil {
			return &ValidationError{Name: "name_en", err: fmt.Errorf(`entscheme: validator failed for field "SysDistrict.name_en": %w`, err)}
		}
	}
	if v, ok := sdc.mutation.Sname(); ok {
		if err := sysdistrict.SnameValidator(v); err != nil {
			return &ValidationError{Name: "sname", err: fmt.Errorf(`entscheme: validator failed for field "SysDistrict.sname": %w`, err)}
		}
	}
	if v, ok := sdc.mutation.SnameEn(); ok {
		if err := sysdistrict.SnameEnValidator(v); err != nil {
			return &ValidationError{Name: "sname_en", err: fmt.Errorf(`entscheme: validator failed for field "SysDistrict.sname_en": %w`, err)}
		}
	}
	if v, ok := sdc.mutation.Abbr(); ok {
		if err := sysdistrict.AbbrValidator(v); err != nil {
			return &ValidationError{Name: "abbr", err: fmt.Errorf(`entscheme: validator failed for field "SysDistrict.abbr": %w`, err)}
		}
	}
	if v, ok := sdc.mutation.StCode(); ok {
		if err := sysdistrict.StCodeValidator(v); err != nil {
			return &ValidationError{Name: "st_code", err: fmt.Errorf(`entscheme: validator failed for field "SysDistrict.st_code": %w`, err)}
		}
	}
	if v, ok := sdc.mutation.Initials(); ok {
		if err := sysdistrict.InitialsValidator(v); err != nil {
			return &ValidationError{Name: "initials", err: fmt.Errorf(`entscheme: validator failed for field "SysDistrict.initials": %w`, err)}
		}
	}
	if v, ok := sdc.mutation.Pinyin(); ok {
		if err := sysdistrict.PinyinValidator(v); err != nil {
			return &ValidationError{Name: "pinyin", err: fmt.Errorf(`entscheme: validator failed for field "SysDistrict.pinyin": %w`, err)}
		}
	}
	if v, ok := sdc.mutation.ParentID(); ok {
		if err := sysdistrict.ParentIDValidator(v); err != nil {
			return &ValidationError{Name: "parent_id", err: fmt.Errorf(`entscheme: validator failed for field "SysDistrict.parent_id": %w`, err)}
		}
	}
	if v, ok := sdc.mutation.AreaCode(); ok {
		if err := sysdistrict.AreaCodeValidator(v); err != nil {
			return &ValidationError{Name: "area_code", err: fmt.Errorf(`entscheme: validator failed for field "SysDistrict.area_code": %w`, err)}
		}
	}
	if v, ok := sdc.mutation.ZipCode(); ok {
		if err := sysdistrict.ZipCodeValidator(v); err != nil {
			return &ValidationError{Name: "zip_code", err: fmt.Errorf(`entscheme: validator failed for field "SysDistrict.zip_code": %w`, err)}
		}
	}
	if v, ok := sdc.mutation.MergeName(); ok {
		if err := sysdistrict.MergeNameValidator(v); err != nil {
			return &ValidationError{Name: "merge_name", err: fmt.Errorf(`entscheme: validator failed for field "SysDistrict.merge_name": %w`, err)}
		}
	}
	if v, ok := sdc.mutation.MergeSname(); ok {
		if err := sysdistrict.MergeSnameValidator(v); err != nil {
			return &ValidationError{Name: "merge_sname", err: fmt.Errorf(`entscheme: validator failed for field "SysDistrict.merge_sname": %w`, err)}
		}
	}
	if v, ok := sdc.mutation.Extra(); ok {
		if err := sysdistrict.ExtraValidator(v); err != nil {
			return &ValidationError{Name: "extra", err: fmt.Errorf(`entscheme: validator failed for field "SysDistrict.extra": %w`, err)}
		}
	}
...
func (u *SysDistrictUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysDistrictCreate.OnConflict")
	}
...
func (u *SysDistrictUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("entscheme: SysDistrictUpsertOne.ID is not supported by MySQL driver. Use SysDistrictUpsertOne.Exec instead")
	}
...
func (u *SysDistrictUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("entscheme: OnConflict was set for builder %d. Set it on the SysDistrictCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysDistrictCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
