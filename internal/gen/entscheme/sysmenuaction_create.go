...
func (smac *SysMenuActionCreate) check() error {
	if _, ok := smac.mutation.IsDel(); !ok {
		return &ValidationError{Name: "is_del", err: errors.New(`entscheme: missing required field "SysMenuAction.is_del"`)}
	}
	if _, ok := smac.mutation.Sort(); !ok {
		return &ValidationError{Name: "sort", err: errors.New(`entscheme: missing required field "SysMenuAction.sort"`)}
	}
	if _, ok := smac.mutation.IsActive(); !ok {
		return &ValidationError{Name: "is_active", err: errors.New(`entscheme: missing required field "SysMenuAction.is_active"`)}
	}
	if v, ok := smac.mutation.Memo(); ok {
		if err := sysmenuaction.MemoValidator(v); err != nil {
			return &ValidationError{Name: "memo", err: fmt.Errorf(`entscheme: validator failed for field "SysMenuAction.memo": %w`, err)}
		}
	}
	if _, ok := smac.mutation.MenuID(); !ok {
		return &ValidationError{Name: "menu_id", err: errors.New(`entscheme: missing required field "SysMenuAction.menu_id"`)}
	}
	if v, ok := smac.mutation.MenuID(); ok {
		if err := sysmenuaction.MenuIDValidator(v); err != nil {
			return &ValidationError{Name: "menu_id", err: fmt.Errorf(`entscheme: validator failed for field "SysMenuAction.menu_id": %w`, err)}
		}
	}
	if _, ok := smac.mutation.Code(); !ok {
		return &ValidationError{Name: "code", err: errors.New(`entscheme: missing required field "SysMenuAction.code"`)}
	}
	if v, ok := smac.mutation.Code(); ok {
		if err := sysmenuaction.CodeValidator(v); err != nil {
			return &ValidationError{Name: "code", err: fmt.Errorf(`entscheme: validator failed for field "SysMenuAction.code": %w`, err)}
		}
	}
	if _, ok := smac.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`entscheme: missing required field "SysMenuAction.name"`)}
	}
	if v, ok := smac.mutation.Name(); ok {
		if err := sysmenuaction.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`entscheme: validator failed for field "SysMenuAction.name": %w`, err)}
		}
	}
	if v, ok := smac.mutation.ID(); ok {
		if err := sysmenuaction.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`entscheme: validator failed for field "SysMenuAction.id": %w`, err)}
		}
	}
	return nil
}
...
func (u *SysMenuActionUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysMenuActionCreate.OnConflict")
	}
...
func (u *SysMenuActionUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("entscheme: SysMenuActionUpsertOne.ID is not supported by MySQL driver. Use SysMenuActionUpsertOne.Exec instead")
	}
...
func (u *SysMenuActionUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("entscheme: OnConflict was set for builder %d. Set it on the SysMenuActionCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysMenuActionCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
