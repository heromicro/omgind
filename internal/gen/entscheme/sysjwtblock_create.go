...
func (sjbc *SysJwtBlockCreate) check() error {
	if _, ok := sjbc.mutation.IsDel(); !ok {
		return &ValidationError{Name: "is_del", err: errors.New(`entscheme: missing required field "SysJwtBlock.is_del"`)}
	}
	...
	if _, ok := sjbc.mutation.IsActive(); !ok {
		return &ValidationError{Name: "is_active", err: errors.New(`entscheme: missing required field "SysJwtBlock.is_active"`)}
	}
	if _, ok := sjbc.mutation.Jwt(); !ok {
		return &ValidationError{Name: "jwt", err: errors.New(`entscheme: missing required field "SysJwtBlock.jwt"`)}
	}
	...
	if v, ok := sjbc.mutation.ID(); ok {
		if err := sysjwtblock.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`entscheme: validator failed for field "SysJwtBlock.id": %w`, err)}
		}
	}
	return nil
}
...
func (u *SysJwtBlockUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysJwtBlockCreate.OnConflict")
	}
...
func (u *SysJwtBlockUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("entscheme: SysJwtBlockUpsertOne.ID is not supported by MySQL driver. Use SysJwtBlockUpsertOne.Exec instead")
	}
...
func (u *SysJwtBlockUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("entscheme: OnConflict was set for builder %d. Set it on the SysJwtBlockCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysJwtBlockCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
