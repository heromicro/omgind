...
func (surc *SysUserRoleCreate) check() error {
	if _, ok := surc.mutation.IsDel(); !ok {
		return &ValidationError{Name: "is_del", err: errors.New(`entscheme: missing required field "SysUserRole.is_del"`)}
	}
	if _, ok := surc.mutation.UserID(); !ok {
		return &ValidationError{Name: "user_id", err: errors.New(`entscheme: missing required field "SysUserRole.user_id"`)}
	}
	if v, ok := surc.mutation.UserID(); ok {
		if err := sysuserrole.UserIDValidator(v); err != nil {
			return &ValidationError{Name: "user_id", err: fmt.Errorf(`entscheme: validator failed for field "SysUserRole.user_id": %w`, err)}
		}
	}
	if _, ok := surc.mutation.RoleID(); !ok {
		return &ValidationError{Name: "role_id", err: errors.New(`entscheme: missing required field "SysUserRole.role_id"`)}
	}
	if v, ok := surc.mutation.RoleID(); ok {
		if err := sysuserrole.RoleIDValidator(v); err != nil {
			return &ValidationError{Name: "role_id", err: fmt.Errorf(`entscheme: validator failed for field "SysUserRole.role_id": %w`, err)}
		}
	}
	if v, ok := surc.mutation.ID(); ok {
		if err := sysuserrole.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`entscheme: validator failed for field "SysUserRole.id": %w`, err)}
		}
	}
	return nil
}
...
func (u *SysUserRoleUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysUserRoleCreate.OnConflict")
	}
...
func (u *SysUserRoleUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("entscheme: SysUserRoleUpsertOne.ID is not supported by MySQL driver. Use SysUserRoleUpsertOne.Exec instead")
	}
...
func (u *SysUserRoleUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("entscheme: OnConflict was set for builder %d. Set it on the SysUserRoleCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysUserRoleCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
