...
func (srmc *SysRoleMenuCreate) check() error {
	if _, ok := srmc.mutation.IsDel(); !ok {
		return &ValidationError{Name: "is_del", err: errors.New(`entscheme: missing required field "SysRoleMenu.is_del"`)}
	}
	if _, ok := srmc.mutation.RoleID(); !ok {
		return &ValidationError{Name: "role_id", err: errors.New(`entscheme: missing required field "SysRoleMenu.role_id"`)}
	}
	if v, ok := srmc.mutation.RoleID(); ok {
		if err := sysrolemenu.RoleIDValidator(v); err != nil {
			return &ValidationError{Name: "role_id", err: fmt.Errorf(`entscheme: validator failed for field "SysRoleMenu.role_id": %w`, err)}
		}
	}
	if _, ok := srmc.mutation.MenuID(); !ok {
		return &ValidationError{Name: "menu_id", err: errors.New(`entscheme: missing required field "SysRoleMenu.menu_id"`)}
	}
	if v, ok := srmc.mutation.MenuID(); ok {
		if err := sysrolemenu.MenuIDValidator(v); err != nil {
			return &ValidationError{Name: "menu_id", err: fmt.Errorf(`entscheme: validator failed for field "SysRoleMenu.menu_id": %w`, err)}
		}
	}
	if v, ok := srmc.mutation.ActionID(); ok {
		if err := sysrolemenu.ActionIDValidator(v); err != nil {
			return &ValidationError{Name: "action_id", err: fmt.Errorf(`entscheme: validator failed for field "SysRoleMenu.action_id": %w`, err)}
		}
	}
	if v, ok := srmc.mutation.ID(); ok {
		if err := sysrolemenu.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`entscheme: validator failed for field "SysRoleMenu.id": %w`, err)}
		}
	}
	return nil
}
...
func (u *SysRoleMenuUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysRoleMenuCreate.OnConflict")
	}
...
func (u *SysRoleMenuUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("entscheme: SysRoleMenuUpsertOne.ID is not supported by MySQL driver. Use SysRoleMenuUpsertOne.Exec instead")
	}
...
func (u *SysRoleMenuUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("entscheme: OnConflict was set for builder %d. Set it on the SysRoleMenuCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysRoleMenuCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
