...
func (stc *SysTeamCreate) check() error {
	if _, ok := stc.mutation.Sort(); !ok {
		return &ValidationError{Name: "sort", err: errors.New(`entscheme: missing required field "SysTeam.sort"`)}
	}
	if _, ok := stc.mutation.IsActive(); !ok {
		return &ValidationError{Name: "is_active", err: errors.New(`entscheme: missing required field "SysTeam.is_active"`)}
	}
	if v, ok := stc.mutation.Memo(); ok {
		if err := systeam.MemoValidator(v); err != nil {
			return &ValidationError{Name: "memo", err: fmt.Errorf(`entscheme: validator failed for field "SysTeam.memo": %w`, err)}
		}
	}
	if _, ok := stc.mutation.IsDel(); !ok {
		return &ValidationError{Name: "is_del", err: errors.New(`entscheme: missing required field "SysTeam.is_del"`)}
	}
	if v, ok := stc.mutation.Name(); ok {
		if err := systeam.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`entscheme: validator failed for field "SysTeam.name": %w`, err)}
		}
	}
	if v, ok := stc.mutation.Code(); ok {
		if err := systeam.CodeValidator(v); err != nil {
			return &ValidationError{Name: "code", err: fmt.Errorf(`entscheme: validator failed for field "SysTeam.code": %w`, err)}
		}
	}
	if v, ok := stc.mutation.ID(); ok {
		if err := systeam.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`entscheme: validator failed for field "SysTeam.id": %w`, err)}
		}
	}
	return nil
}
...
func (u *SysTeamUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysTeamCreate.OnConflict")
	}
...
func (u *SysTeamUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("entscheme: SysTeamUpsertOne.ID is not supported by MySQL driver. Use SysTeamUpsertOne.Exec instead")
	}
...
func (u *SysTeamUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("entscheme: OnConflict was set for builder %d. Set it on the SysTeamCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysTeamCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
