...
func (stuu *SysTeamUserUpdate) check() error {
	...
	if _, ok := stuu.mutation.UserID(); stuu.mutation.UserCleared() && !ok {
		return errors.New(`entscheme: clearing a required unique edge "SysTeamUser.user"`)
	}
	if _, ok := stuu.mutation.TeamID(); stuu.mutation.TeamCleared() && !ok {
		return errors.New(`entscheme: clearing a required unique edge "SysTeamUser.team"`)
	}
	return nil
}
...
func (stuuo *SysTeamUserUpdateOne) sqlSave(ctx context.Context) (_node *SysTeamUser, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`entscheme: missing "SysTeamUser.id" for update`)}
	}
