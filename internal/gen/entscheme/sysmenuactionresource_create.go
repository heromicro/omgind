...
func (smarc *SysMenuActionResourceCreate) check() error {
	if _, ok := smarc.mutation.IsDel(); !ok {
		return &ValidationError{Name: "is_del", err: errors.New(`entscheme: missing required field "SysMenuActionResource.is_del"`)}
	}
	if _, ok := smarc.mutation.Sort(); !ok {
		return &ValidationError{Name: "sort", err: errors.New(`entscheme: missing required field "SysMenuActionResource.sort"`)}
	}
	if v, ok := smarc.mutation.Memo(); ok {
		if err := sysmenuactionresource.MemoValidator(v); err != nil {
			return &ValidationError{Name: "memo", err: fmt.Errorf(`entscheme: validator failed for field "SysMenuActionResource.memo": %w`, err)}
		}
	}
	if _, ok := smarc.mutation.IsActive(); !ok {
		return &ValidationError{Name: "is_active", err: errors.New(`entscheme: missing required field "SysMenuActionResource.is_active"`)}
	}
	if _, ok := smarc.mutation.Method(); !ok {
		return &ValidationError{Name: "method", err: errors.New(`entscheme: missing required field "SysMenuActionResource.method"`)}
	}
	if v, ok := smarc.mutation.Method(); ok {
		if err := sysmenuactionresource.MethodValidator(v); err != nil {
			return &ValidationError{Name: "method", err: fmt.Errorf(`entscheme: validator failed for field "SysMenuActionResource.method": %w`, err)}
		}
	}
	if _, ok := smarc.mutation.Path(); !ok {
		return &ValidationError{Name: "path", err: errors.New(`entscheme: missing required field "SysMenuActionResource.path"`)}
	}
	if v, ok := smarc.mutation.Path(); ok {
		if err := sysmenuactionresource.PathValidator(v); err != nil {
			return &ValidationError{Name: "path", err: fmt.Errorf(`entscheme: validator failed for field "SysMenuActionResource.path": %w`, err)}
		}
	}
	if _, ok := smarc.mutation.ActionID(); !ok {
		return &ValidationError{Name: "action_id", err: errors.New(`entscheme: missing required field "SysMenuActionResource.action_id"`)}
	}
	if v, ok := smarc.mutation.ActionID(); ok {
		if err := sysmenuactionresource.ActionIDValidator(v); err != nil {
			return &ValidationError{Name: "action_id", err: fmt.Errorf(`entscheme: validator failed for field "SysMenuActionResource.action_id": %w`, err)}
		}
	}
	if v, ok := smarc.mutation.ID(); ok {
		if err := sysmenuactionresource.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`entscheme: validator failed for field "SysMenuActionResource.id": %w`, err)}
		}
	}
	return nil
}
...
func (u *SysMenuActionResourceUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysMenuActionResourceCreate.OnConflict")
	}
...
func (u *SysMenuActionResourceUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("entscheme: SysMenuActionResourceUpsertOne.ID is not supported by MySQL driver. Use SysMenuActionResourceUpsertOne.Exec instead")
	}
...
func (u *SysMenuActionResourceUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("entscheme: OnConflict was set for builder %d. Set it on the SysMenuActionResourceCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("entscheme: missing options for SysMenuActionResourceCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
