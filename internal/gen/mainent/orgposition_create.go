...
func (opc *OrgPositionCreate) check() error {
	if _, ok := opc.mutation.IsDel(); !ok {
		return &ValidationError{Name: "is_del", err: errors.New(`mainent: missing required field "OrgPosition.is_del"`)}
	}
	if _, ok := opc.mutation.Sort(); !ok {
		return &ValidationError{Name: "sort", err: errors.New(`mainent: missing required field "OrgPosition.sort"`)}
	}
	if _, ok := opc.mutation.IsActive(); !ok {
		return &ValidationError{Name: "is_active", err: errors.New(`mainent: missing required field "OrgPosition.is_active"`)}
	}
	if v, ok := opc.mutation.Memo(); ok {
		if err := orgposition.MemoValidator(v); err != nil {
			return &ValidationError{Name: "memo", err: fmt.Errorf(`mainent: validator failed for field "OrgPosition.memo": %w`, err)}
		}
	}
	if v, ok := opc.mutation.Name(); ok {
		if err := orgposition.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`mainent: validator failed for field "OrgPosition.name": %w`, err)}
		}
	}
	if v, ok := opc.mutation.Code(); ok {
		if err := orgposition.CodeValidator(v); err != nil {
			return &ValidationError{Name: "code", err: fmt.Errorf(`mainent: validator failed for field "OrgPosition.code": %w`, err)}
		}
	}
	if v, ok := opc.mutation.OrgID(); ok {
		if err := orgposition.OrgIDValidator(v); err != nil {
			return &ValidationError{Name: "org_id", err: fmt.Errorf(`mainent: validator failed for field "OrgPosition.org_id": %w`, err)}
		}
	}
	if v, ok := opc.mutation.ID(); ok {
		if err := orgposition.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`mainent: validator failed for field "OrgPosition.id": %w`, err)}
		}
	}
	return nil
}
...
func (u *OrgPositionUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("mainent: missing options for OrgPositionCreate.OnConflict")
	}
...
func (u *OrgPositionUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("mainent: OrgPositionUpsertOne.ID is not supported by MySQL driver. Use OrgPositionUpsertOne.Exec instead")
	}
...
func (u *OrgPositionUpsertBulk) Exec(ctx context.Context) error {
	...
	if len(u.create.conflict) == 0 {
		return errors.New("mainent: missing options for OrgPositionCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
