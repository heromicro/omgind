...
func (ooc *OrgOrganCreate) check() error {
	if _, ok := ooc.mutation.IsDel(); !ok {
		return &ValidationError{Name: "is_del", err: errors.New(`ent: missing required field "OrgOrgan.is_del"`)}
	}
	if _, ok := ooc.mutation.Sort(); !ok {
		return &ValidationError{Name: "sort", err: errors.New(`ent: missing required field "OrgOrgan.sort"`)}
	}
	if _, ok := ooc.mutation.IsActive(); !ok {
		return &ValidationError{Name: "is_active", err: errors.New(`ent: missing required field "OrgOrgan.is_active"`)}
	}
...
func (u *OrgOrganUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for OrgOrganCreate.OnConflict")
	}
...
func (u *OrgOrganUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("ent: OrgOrganUpsertOne.ID is not supported by MySQL driver. Use OrgOrganUpsertOne.Exec instead")
	}
...
func (u *OrgOrganUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent: OnConflict was set for builder %d. Set it on the OrgOrganCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for OrgOrganCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
