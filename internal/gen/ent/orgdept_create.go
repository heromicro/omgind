...
func (odc *OrgDeptCreate) check() error {
	if _, ok := odc.mutation.IsDel(); !ok {
		return &ValidationError{Name: "is_del", err: errors.New(`ent: missing required field "OrgDept.is_del"`)}
	}
	if _, ok := odc.mutation.Sort(); !ok {
		return &ValidationError{Name: "sort", err: errors.New(`ent: missing required field "OrgDept.sort"`)}
	}
	if _, ok := odc.mutation.IsActive(); !ok {
		return &ValidationError{Name: "is_active", err: errors.New(`ent: missing required field "OrgDept.is_active"`)}
	}
	...
	if v, ok := odc.mutation.ID(); ok {
		if err := orgdept.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`ent: validator failed for field "OrgDept.id": %w`, err)}
		}
	}
	return nil
}
...
func (odc *OrgDeptCreate) createSpec() (*OrgDept, *sqlgraph.CreateSpec) {
	...
	_spec.Schema = odc.schemaConfig.OrgDept
	...
	if nodes := odc.mutation.ParentIDs(); len(nodes) > 0 {
		...
		edge.Schema = odc.schemaConfig.OrgDept
		for _, k := range nodes {
...
func (u *OrgDeptUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for OrgDeptCreate.OnConflict")
	}
...
func (u *OrgDeptUpsertOne) ID(ctx context.Context) (id string, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("ent: OrgDeptUpsertOne.ID is not supported by MySQL driver. Use OrgDeptUpsertOne.Exec instead")
	}
...
func (u *OrgDeptUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent: OnConflict was set for builder %d. Set it on the OrgDeptCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for OrgDeptCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
